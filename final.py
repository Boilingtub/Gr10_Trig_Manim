...
class Test(Scene):
    def construct(self):
        for quad in range(1,5):
            ...
            tan_color = GOLD if (y_sign*x_sign > 0) else BLUE
            sin_color = GOLD if (y_sign > 0) else BLUE
            cos_color = GOLD if (x_sign > 0) else BLUE

            tan_height = 0.35 if (y_sign > 0) else 0.85
            sin_height = 0.85 if (y_sign > 0) else 0.35




            funcs = [
                MathTex(r"\sin(\theta) = \frac{" + y_char + r"y}{r} \text{ is } " + sin_sign,
                        color=sin_color,font_size=10,

                ).shift(RIGHT*1.3*x_sign + UP*sin_height*y_sign),
                MathTex(r"\cos(\theta) = \frac{" + x_char + r"x}{r} \text{ is } " + cos_sign,
                        color=cos_color,font_size=10,

                ).shift(RIGHT*1.3*x_sign + UP*0.6*y_sign),
                MathTex(r"\tan(\theta) = \frac{" + y_char + r"y}{" + x_char + r"x} \text{ is } " + tan_sign,
                        color=tan_color,font_size=10,
                ).shift(RIGHT*1.3*x_sign + UP*tan_height*y_sign),
            ]

            self.play(Write(funcs[0]), Write(funcs[1]), Write(funcs[2]))
...
class TrigLesson(Scene):
    def construct(self):
        ...
        for quad in range(0,4):
            self.play(
                    Unwrite(free_later_objs[0][0]),
                    Unwrite(free_later_objs[0][1]),
                    Unwrite(free_later_objs[0][2]),
                    Unwrite(free_later_objs[0][3]),
                    Unwrite(free_later_objs[0][4]),
                    Unwrite(free_later_objs[0][5]),

                    Unwrite(free_later_objs[1][0]),
                    Unwrite(free_later_objs[1][1]),
                    Unwrite(free_later_objs[1][2]),
                    Unwrite(free_later_objs[1][3]),
                    Unwrite(free_later_objs[1][4]),
                    Unwrite(free_later_objs[1][5]),

                    Unwrite(free_later_objs[2][0]),
                    Unwrite(free_later_objs[2][1]),
                    Unwrite(free_later_objs[2][2]),
                    Unwrite(free_later_objs[2][3]),
                    Unwrite(free_later_objs[2][4]),
                    Unwrite(free_later_objs[2][5]),

                    Unwrite(free_later_objs[3][0]),
                    Unwrite(free_later_objs[3][1]),
                    Unwrite(free_later_objs[3][2]),
                    Unwrite(free_later_objs[3][3]),
                    Unwrite(free_later_objs[3][4]),
                    Unwrite(free_later_objs[3][5]),
            )
        ...
        for quad in range(0,4):
            self.play(Unwrite(free_later_tex[quad][0]),
                      Unwrite(free_later_tex[quad][1]),
                      Unwrite(free_later_tex[quad][2]),
                      Unwrite(quadrant_label[quad]),
                      run_time=0.5
            )
        ...
        desc_text = Tex(
            r"\text{You now know you must turn } \textbf{30 degrees clockwise} \text{ and walk } \textbf{4 kilometers} \\" +
            r"\text{You call your friend on the walkie talkie and} \\",
            r"\text{tell him to turn } \textbf{45 degrees counter clockwise} \text{ and walk } \textbf{2.8284 kilometers} \\",
